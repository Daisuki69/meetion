// Session configs — in production these come from your server by meet code.
// startTime: ISO string of when the meeting goes live.
//            null = dev mode (meeting is always "live" right now)
// earlyEntry: how many seconds before startTime the professor tile appears (-600 = 10min early)

export const MOCK_SESSIONS = {
  "abc-defg-hij": {
    code: "abc-defg-hij",
    title: "CS301 - Algorithms & Data Structures",
    startTime: null,         // Set to ISO string like "2025-09-15T09:00:00" for production
    earlyEntrySeconds: 600,  // Professor appears 10min before start
    professor: {
      id: "prof-001",
    ...
    // Scripted events — seconds from meeting start (not from earlyEntry window)
    events: [
...
/**
 * Returns the current meeting state given a session config.
 *
 * States:
 *   "too_early"  — before earlyEntry window (more than earlyEntrySeconds before start)
 *   "early"      — in early window: professor only, no participants yet
 *   "live"       — meeting is running
 *   "dev"        — startTime is null, always treat as live at elapsed=0
 *
 * Returns { state, elapsedSeconds }
 * elapsedSeconds: seconds since meeting START (0 at startTime, negative in early window)
 */
export function getMeetingState(session) {
  if (!session.startTime) {
    return { state: "dev", elapsedSeconds: 0 }
  }
  ...
  const earlyMs = session.earlyEntrySeconds * 1000

  if (elapsedMs < -earlyMs) {
    return { state: "too_early", elapsedSeconds }
  }
  if (elapsedMs < 0) {
    return { state: "early", elapsedSeconds }  // negative, e.g. -300 = 5min before start
  }
  return { state: "live", elapsedSeconds }
}

/**
 * Resolve random join/leave times for participants.
 * Called once when the meeting screen mounts — produces consistent times for the session.
 */
export function resolveParticipantSchedule(participants) {
